# 🛡️ Guardião Digital da Operação — Vale

Jogo web **bilíngue (Português 🇧🇷 / Inglês 🇬🇧)** de conscientização em **Cyber Security e Segurança da Informação**, ambientado nas operações reais da Vale ao redor do mundo. Funciona 100% no navegador, sem login.

---

## ✨ Destaques desta versão

- **🗺️ Mapa “A Vale no mundo”** — componente em `js/vale-world-map.js` (D3 + TopoJSON + `assets/countries-110m.json`), alinhado ao mapa oficial de [vale.com/pt/onde-estamos](https://www.vale.com/pt/onde-estamos). Legendas de **Atuação** (11) e **Portfólio** (7), filtro clicável, tooltip e lista de países. **Ícones 100% offline** (SVG inline — sem dependência de vale.com).
- **⛓️ Cadeia Carajás → China** — aba **Cadeia ferro** no mapa: animação vetorial da mina S11D à China (ferrovia EFC, porto Ponta da Madeira, navio Valemax).
...
```
.
├── index.html                 # Telas, mapa Vale, taskbar inferior
├── README.md
...
└── js/
    ...
    ├── vale-world-map.js      # Motor do mapa oficial Vale (D3/TopoJSON)
    └── sign-lang.js           # Integração VLibras/ASL
```

> A lógica principal permanece em **`js/game.js`**. O mapa mundial está em **`js/vale-world-map.js`** (D3/TopoJSON). Versão de cache: `window.APP_VERSION` em `index.html` (atualmente **26**).
...
Ferramenta **educativa interna**. O mapa e as operações são referências públicas (vale.com/pt/onde-estamos). O certificado é ilustrativo e **não substitui certificações oficiais**. Siga sempre as políticas oficiais de segurança da Vale.
