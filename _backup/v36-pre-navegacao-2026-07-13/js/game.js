...
var DEF = { lang:"pt", name:"", team:"mina", role:"admin",
  ...
  chainDone:{}, streak:{count:0,lastDate:"",best:0}, missed:{}, reports:0 };
...
var UI = {
  ...
  "home.mission":{pt:"🎯 <b>Sua missão:</b> defender minas, ferrovias, portos e escritórios da Vale pelo mundo. Explore o mapa global, enfrente os <b>chefões</b>, cumpra desafios diários e semanais e evolua sua carreira.",en:"🎯 <b>Your mission:</b> defend Vale's mines, railways, ports and offices worldwide. Explore the global map, face <b>bosses</b>, complete daily and weekly challenges, and grow your career."},
  ...
  "home.heroIronTip":{pt:"Clique para abrir a cadeia de ferro animada no mapa — mina, ferrovia, porto e navio.",en:"Click to open the animated iron chain on the map — mine, railway, port and ship."},
  "home.chain":{pt:"⛓️ Cadeia de Produção",en:"⛓️ Production Chain"},
  "mode.mapchain.h":{pt:"Cadeia no mapa",en:"Chain on the map"},
  "mode.mapchain.t":{pt:"Etapas de Carajás à China no mapa global — clique para proteger cada elo.",en:"Stages from Carajás to China on the global map — click to protect each link."},
  ...
  "map.resilience":{pt:"Resiliência operacional",en:"Operational resilience"},
  ...
  "quiz.resilience":{pt:"🛡️ Resiliência operacional",en:"🛡️ Operational resilience"},
  ...
  "boss.ops":{pt:"🏭 Resiliência operacional",en:"🏭 Operational resilience"},
  ...
  "profile.bossRes":{pt:"🏭 Resiliência nos Chefões",en:"🏭 Boss Resilience"},
  ...
  "onboard.langT":{pt:"Escolha o idioma",en:"Choose your language"},
  "onboard.langB":{pt:"O jogo, as perguntas, as respostas e a narração seguem o idioma escolhido. Você pode alterar depois no topo (🌐) ou neste guia.",en:"The game, questions, answers and narration follow the language you choose. You can change it later at the top (🌐) or in this guide."},
  "onboard.langSub":{pt:"Toque para selecionar / Tap to select",en:"Tap to select / Toque para selecionar"},
  "onboard.s1t":{pt:"Bem-vindo, Guardião!",en:"Welcome, Guardian!"},
  ...
  "onboard.s2t":{pt:"Mapa e campanhas",en:"Map and campaigns"},
  ...
  "onboard.s3t":{pt:"Chefões tabletop",en:"Tabletop bosses"},
  "onboard.s3b":{pt:"Nas aventuras estilo mesa, 10 cenas conectadas simulam crises reais. Erros comprometem a operação; acertos aumentam sua maturidade em resiliência.",en:"In tabletop adventures, 10 connected scenes simulate real crises. Mistakes compromise operations; correct choices build resilience maturity."},
  "onboard.s4t":{pt:"Progresso e certificado",en:"Progress and certificate"},
  ...
  "progress.map":{pt:"Mapa / Cadeia",en:"Map / Chain"},
  "progress.mapD":{pt:"Campanhas por país + etapas Carajás→China. Atualiza países, radar de temas e semanal “campanhas”.",en:"Country campaigns + Carajás→China stages. Updates countries, theme radar and weekly “campaigns”."},
  ...
  "progress.bossD":{pt:"10 cenas tabletop com maturidade operacional. Alimenta resiliência, conquistas e selo do certificado.",en:"10 tabletop scenes with operational maturity. Feeds resilience, achievements and certificate seal."},
  ...
  "nav.tip.map":{pt:"Mapa — países, cadeias e processos Vale",en:"Map — countries, chains and Vale processes"},
  "nav.tip.daily":{pt:"Missões diárias",en:"Daily missions"},
  "nav.tip.more":{pt:"Mais modos — chefões, semanal, loja…",en:"More modes — bosses, weekly, shop…"},
...
function applyHudTips(){
  var map={
    ...
    hudCoinsChip:"hud.tip.coins", hudScoreChip:"hud.tip.score",
...
function setLang(lang){
  ...
  document.querySelectorAll(".onboard-lang-btn").forEach(function(x){ x.setAttribute("aria-pressed",x.getAttribute("data-lang")===lang?"true":"false"); });
...
/* -------------------- NAVEGAÇÃO -------------------- */
var NAVMAP={screenHome:"navHomeBtn",screenMap:"navMapBtn",screenBossList:"navBossBtn",screenDaily:"navDailyBtn",screenWeekly:"navWeeklyBtn",screenShop:"navShopBtn",screenProfile:"navStatsBtn",screenManager:"navManagerBtn"};
...
function announce(m){ $("a11yLive").textContent=""; setTimeout(function(){ $("a11yLive").textContent=m; },40); }
...
function refreshHud(){
  $("hudXp").textContent=S.xp; $("hudCoins").textContent=S.coins; $("hudScore").textContent=S.score;
  $("hudLevel").textContent=levelOf();
  var ti=currentTitle(); $("hudTitle").textContent=ti.ico+" "+tt(ti);
  ensureStreak();
  var hs=$("hudStreak"); if(hs) hs.textContent="🔥 "+(S.streak.count||0);
  applyCosmetics();
  renderLives();
}
function renderLives(){ var n=Math.max(0,S.lives||0); $("hudLives").textContent="❤️".repeat(n)+"🖤".repeat(Math.max(0,3-n)); }
...
function initQuizSession(){ cur.sessionLog=[]; cur.reportPending=false; cur.reportDone=false; var rs=$("reportStep"); if(rs) rs.hidden=true; }
function showReportPrompt(q){
  var rs=$("reportStep"); if(!rs) return;
  cur.reportPending=true; $("nextBtn").style.display="none"; rs.hidden=false;
  rs.innerHTML='<div class="report-title">'+t("report.title")+'</div><p class="report-sub">'+t("report.sub")+'</p><div class="report-btns"></div>';
  ...
  sk.addEventListener("click",function(){ cur.reportPending=false; rs.hidden=true; $("nextBtn").style.display="inline-block"; $("nextBtn").focus(); });
  box.appendChild(sk);
}
function completeReport(){ S.reports=(S.reports||0)+1; addReward(5,2,0); save(); cur.reportPending=false; var rs=$("reportStep");
  if(rs){ rs.innerHTML='<p class="report-done">'+t("report.done")+'</p>'; } $("nextBtn").style.display="inline-block"; $("nextBtn").focus(); toast(t("report.done")); }
...
function ensureTeamScores(){
  TEAMS.forEach(function(tm){ if(S.teamScores[tm.id]===undefined) S.teamScores[tm.id]=40+Math.floor(Math.random()*35); });
}
function renderRank(host){
  var acc=overallAcc()||0;
  var arr=TEAMS.map(function(tm){ return {tm:tm,val:tm.id===S.team?acc:null}; });
  arr.sort(function(a,b){ return (b.val||0)-(a.val||0); });
  host.innerHTML="";
  arr.forEach(function(r,i){
    var d=document.createElement("div"); d.className="rank-item"+(r.tm.id===S.team?" you":"");
    d.innerHTML='<span class="rank-pos">'+(i+1)+'º</span><span>'+r.tm.ico+' '+tt(r.tm)+'</span><span class="rank-bar"><span class="rf" style="width:'+(r.val!==null?r.val:0)+'%"></span></span><span class="rank-val">'+(r.val!==null?r.val+'%':'—')+'</span>';
    host.appendChild(d);
  });
}
...
var MAP_PROCESSES=[
  {id:null, label:"map.procWorld", tip:"map.procWorldTip"},
  {id:"iron", label:"map.procIron", tip:"map.procIronTip"}
];
function normalizeMapProcess(){
  if(mapProcess==="iron"||mapProcess===null) return;
  var ok=false;
  MAP_PROCESSES.forEach(function(p){ if(p.id===mapProcess) ok=true; });
  if(!ok) mapProcess=null;
}
function mapChainMode(){ return mapProcess==="iron"; }
...
function updateMapContext(){
  ...
  var sw=$("mapSearchWrap"); if(sw) sw.hidden=!!mapChainMode();
}
...
function updateMapHint(){
  ...
  h.textContent=mapProcess==="iron"?t("map.hintIron"):t("map.hint");
}
function setChainStageHighlight(id, scroll){
  chainStageActive=id||null;
  document.querySelectorAll("#chainFlow .chain-stage").forEach(function(el){
    el.classList.toggle("active",!!id&&el.getAttribute("data-stage")===id);
  });
  var anim=$("mapChainAnim");
  if(anim) anim.querySelectorAll(".cmap-pin").forEach(function(el){
    el.classList.toggle("active",!!id&&el.getAttribute("data-stage")===id);
  });
  if(scroll&&id) scrollMapListItem('#chainFlow .chain-stage[data-stage="'+id+'"]');
}
function renderChainStageList(){
  var panel=$("chainListPanel"), flow=$("chainFlow"), cap=$("chainMapCaption"), note=$("chainImpactNote");
  ...
  ch.stages.forEach(function(st,i){
    ...
    b.addEventListener("click",function(){ openMapDetailChain(st.id); });
...
function renderAnimatedChainMap(){
  var host=$("mapChainAnim"), stage=$("mapStage"); if(!host) return;
  var ch=chainById(curChainId);
  ...
  host.innerHTML=svg;
  host.hidden=false;
  if(stage) stage.classList.add("mode-iron");
  var wrap=$("valeWorldMap"); if(wrap) wrap.classList.add("mode-iron-map");
  host.querySelectorAll(".cmap-pin").forEach(function(g){
    var sid=g.getAttribute("data-stage");
    g.addEventListener("click",function(e){ e.stopPropagation(); setChainStageHighlight(sid,true); openMapDetailChain(sid); });
    g.addEventListener("keydown",function(e){ if(e.key==="Enter"||e.key===" "){ e.preventDefault(); openMapDetailChain(sid); } });
...
function hideAnimatedChainMap(){
  var host=$("mapChainAnim"), stage=$("mapStage"), wrap=$("valeWorldMap");
  if(host){ host.hidden=true; host.innerHTML=""; }
  if(stage) stage.classList.remove("mode-iron");
  if(wrap) wrap.classList.remove("mode-iron-map");
}
...
function renderMapLegend(){
  ...
  if(mapChainMode()){
    ...
    leg.innerHTML='<span class="ml-item" style="font-weight:700;color:#eaf2f6">'+t("map.legendTitle")+':</span>';
    [{sw:"background:#c47d08",t:"map.legendIron"},{dot:"#EDB111",t:"map.legendChain"}].forEach(function(it){
      var d=document.createElement("span"); d.className="ml-item";
      if(it.sw) d.innerHTML='<span class="ml-swatch" style="'+it.sw+'"></span><span>'+t(it.t)+'</span>';
      else d.innerHTML='<span class="ml-dot" style="background:'+it.dot+'"></span><span>'+t(it.t)+'</span>';
      leg.appendChild(d);
    });
    return;
  }
...
function drawMap(){
  ...
  hideAnimatedChainMap();
  var cl=$("countryList"), chp=$("chainListPanel");
  if(mapChainMode()){
    renderAnimatedChainMap();
    if(cl) cl.hidden=true;
    if(chp){ chp.hidden=false; renderChainStageList(); }
    updateViewBox();
    renderMapLegend();
    renderMapProcessBar();
    updateMapHint();
...
function focusIronChain(){ setMapProcess("iron"); }
...
function renderQuestion(){
  ...
  var order=shuffle(q.opts.map(function(o,idx){ return {o:o,idx:idx}; })); var letters=["A","B","C","D"];
  order.forEach(function(item,pos){ var b=document.createElement("button"); b.className="opt"; b.setAttribute("data-idx",String(item.idx)); if(item.idx===q.correct) b.setAttribute("data-correct","1"); b.innerHTML='<span class="kx">'+letters[pos]+'</span><span>'+tt(item.o)+'</span>'; b.addEventListener("click",function(){ answer(item.idx,b,q); }); opts.appendChild(b); });
  $("feedback").className="feedback"; $("feedback").innerHTML=""; $("nextBtn").style.display="none";
  speak(tt(q.q));
}
...
function answer(idx,btn,q){
  ...
  if(!cur.sessionLog) cur.sessionLog=[];
  cur.sessionLog.push({q:q,ok:ok});
  if(ok){ btn.classList.add("correct"); cur.correct++; if(!review) addReward(10,5,q.diff*10); fb.className="feedback show good"; fb.innerHTML="✅ <b>"+(L()==="pt"?"Correto!":"Correct!")+"</b> "+tt(q.why); }
  ...
  else { cur.reportPending=false; $("nextBtn").style.display="inline-block"; $("nextBtn").focus(); }
}
function nextQuestion(){
  if(cur.reportPending) return;
  cur.i++; if(cur.i>=cur.questions.length||(cur.mode!=="review"&&(S.lives||0)<=0)){ finishCampaign(); return; } renderQuestion();
}
...
function renderBossList(){
  ...
  BOSSES.forEach(function(b){
    ...
    d.type="button";
    d.className="boss-card boss-card--tabletop";
    var meta=b.phases.length+" "+t("boss.scenes")+(b.tag?" · "+tt(b.tag):"");
    ...
    d.innerHTML='<span class="be">'+b.emoji+'</span><div class="boss-card-body"><span class="boss-card-tag">'+t("boss.storyLabel")+'</span><div class="bt">'+tt(b.name)+'</div><div class="bd">'+tt(b.desc)+'</div><div class="boss-card-meta">'+meta+'</div></div>'+badge;
    d.addEventListener("click",function(){ startBoss(b.id); });
    host.appendChild(d);
  });
}
function startBoss(id){ bossCur.boss=BOSSES.filter(function(x){return x.id===id;})[0]; bossCur.phase=0; bossCur.hp=100; bossCur.ops=100; bossCur.answered=false; bossCur.correct=0; bossCur.wrong=0; bossCur.log=[]; renderBossPhase(); show("screenBoss"); }
function renderBossPhase(){
  ...
  if(intro){
    if(bossCur.phase===0&&b.intro){ intro.hidden=false; intro.innerHTML='<span class="boss-story-k">'+t("boss.scenario")+'</span><p>'+tt(b.intro)+'</p>'; }
  ...
  renderBossBars();
  $("bossPhaseNum").textContent=t("boss.scene")+" "+(bossCur.phase+1)+"/"+b.phases.length;
  ...
  var opts=$("bossOptions"); opts.innerHTML=""; bossCur.answered=false;
  var order=shuffle(ph.opts.map(function(o,i){ return {o:o,idx:i}; })); var letters=["A","B","C","D"];
  order.forEach(function(item,pos){ var btn=document.createElement("button"); btn.className="opt"; if(item.idx===ph.correct) btn.setAttribute("data-correct","1"); btn.innerHTML='<span class="kx">'+letters[pos]+'</span><span>'+tt(item.o)+'</span>'; btn.addEventListener("click",function(){ bossAnswer(item.idx,btn,ph); }); opts.appendChild(btn); });
  $("bossFb").className="feedback"; $("bossFb").innerHTML=""; $("bossNext").style.display="none";
  var nxt=$("bossNext");
  if(nxt) nxt.textContent=bossCur.phase>=b.phases.length-1?t("boss.finish"):t("boss.next");
  speak(tt(b.name)+". "+(ph.scene?tt(ph.scene)+". ":"")+tt(ph.q));
}
function bossAnswer(idx,btn,ph){
  ...
  $("bossNext").style.display="inline-block"; $("bossNext").focus();
}
function bossNext(){ bossCur.phase++; if(bossCur.phase>=bossCur.boss.phases.length){ finishBoss(); return; } renderBossPhase(); }
function finishBoss(){
...
function bumpWeekly(id,n){ if(!n) return; ensureWeekly(); S.weekly.prog[id]=(S.weekly.prog[id]||0)+n; var w=WEEKLY.filter(function(x){return x.id===id;})[0]; if(w && S.weekly.prog[id]===w.goal){ addReward(40,20,0); toast((L()==="pt"?"🏆 Semanal concluída: ":"🏆 Weekly done: ")+tt(w.name)); } save(); }
...
function renderProgressHub(){
  ...
  host.innerHTML=
    '<div class="prog-hub-grid">'+
      ...
      '<div class="prog-hub-item"><span class="prog-hub-ico">🐉</span><div><div class="prog-hub-t">'+t("progress.boss")+'</div><div class="prog-hub-d">'+t("progress.bossD")+'</div><div class="prog-hub-m">'+bossCompletedCount()+'/'+BOSSES.length+' · '+bossAvgIndex()+'%</div></div></div>'+
    ...
    '<div class="row-actions" style="margin-top:10px">'+
      '<button class="btn btn-ghost btn-sm" id="reviewErrorsBtn" type="button">'+t("pedagogy.reviewErrors")+'</button>'+
      '<a class="btn btn-ghost btn-sm" href="review.html" target="_blank" rel="noopener">'+t("progress.reviewBank")+'</a>'+
    '</div>';
  var reb=$("reviewErrorsBtn"); if(reb) reb.onclick=startReviewErrors;
}
function renderProfile(){
  var lab=L()==="pt"?{a:"Nível",b:"XP",c:"Moedas",d:"Países",e:"Chefões",f:"Ofensiva",g:"Resiliência",h:"Reportes"}:{a:"Level",b:"XP",c:"Coins",d:"Countries",e:"Bosses",f:"Streak",g:"Resilience",h:"Reports"};
  ensureStreak(); ensureBossStats();
  var avg=bossAvgIndex(), br=bossGuardianRank(avg);
  $("profileStats").innerHTML='<div class="stat"><div class="v">'+levelOf()+'</div><div class="l">'+lab.a+'</div></div><div class="stat"><div class="v">'+S.xp+'</div><div class="l">'+lab.b+'</div></div><div class="stat"><div class="v">'+S.coins+'</div><div class="l">'+lab.c+'</div></div><div class="stat"><div class="v">'+Object.keys(S.done).length+"/"+COUNTRIES.length+'</div><div class="l">'+lab.d+'</div></div><div class="stat"><div class="v">'+bossCompletedCount()+"/"+BOSSES.length+'</div><div class="l">'+lab.e+'</div></div><div class="stat"><div class="v">'+br.ico+' '+avg+'%</div><div class="l">'+lab.g+'</div></div><div class="stat"><div class="v">🔥 '+(S.streak.count||0)+'</div><div class="l">'+lab.f+'</div></div><div class="stat"><div class="v">📢 '+(S.reports||0)+'</div><div class="l">'+lab.h+'</div></div>';
...
function renderManager(){
  ...
  TEAMS.forEach(function(tm){
    var real=tm.id===S.team, val=real?acc:null;
    var d=document.createElement("div"); d.className="mgr-row"+(real?" mgr-row-you":"");
    d.innerHTML='<span class="mname">'+tm.ico+' '+tt(tm)+(real?' <span class="mgr-you-tag">✓</span>':'')+'</span><span class="rank-bar"><span class="rf" style="width:'+(val!==null?val:0)+'%"></span></span><span class="rank-val">'+(val!==null?val+'%':'—')+'</span>';
    host.appendChild(d);
  });
  var pend=$("mgrPending"); if(pend) pend.textContent=t("manager.pending");
...
function renderOnboarding(){
  var step=ONBOARD_STEPS[onboardStep], isLang=step.type==="lang";
  var langPanel=$("onboardLangPanel"), ico=$("onboardIco"), body=$("onboardBody");
  if(ico) ico.hidden=isLang;
  if(body) body.hidden=isLang;
  if(langPanel){
    langPanel.hidden=!isLang;
    if(isLang){
      var sub=$("onboardLangSub"); if(sub) sub.textContent=t("onboard.langSub");
      document.querySelectorAll(".onboard-lang-btn").forEach(function(b){ b.setAttribute("aria-pressed",b.getAttribute("data-lang")===S.lang?"true":"false"); });
    }
  }
  if(ico&&!isLang) ico.textContent=step.ico;
  if($("onboardTitle")) $("onboardTitle").textContent=t(step.titleKey);
  if(body&&!isLang) body.textContent=t(step.bodyKey);
...
function bind(){
  ...
  if($("optVoice")) $("optVoice").addEventListener("change",function(){ S.a11y.voice=this.checked; save(); applyA11y(); if(this.checked) speak(L()==="pt"?"Narração por voz ativada.":"Voice narration enabled."); });
  if($("optContrast")) $("optContrast").addEventListener("change",function(){ S.a11y.contrast=this.checked; save(); applyA11y(); });
  if($("optLarge")) $("optLarge").addEventListener("change",function(){ S.a11y.large=this.checked; save(); applyA11y(); });
  if($("optMotion")) $("optMotion").addEventListener("change",function(){ S.a11y.motion=this.checked; save(); applyA11y(); });
  if($("optSigns")) $("optSigns").addEventListener("change",function(){ S.a11y.signs=this.checked; save(); applyA11y(); });
  if($("optLinks")) $("optLinks").addEventListener("change",function(){ S.a11y.links=this.checked; save(); applyA11y(); });
  if($("optSpacing")) $("optSpacing").addEventListener("change",function(){ S.a11y.spacing=this.checked; save(); applyA11y(); });
  if($("optLetterSpace")) $("optLetterSpace").addEventListener("change",function(){ S.a11y.letterSpace=this.checked; save(); applyA11y(); });
  if($("optDyslexia")) $("optDyslexia").addEventListener("change",function(){ S.a11y.dyslexia=this.checked; save(); applyA11y(); });
  if($("optReadingMode")) $("optReadingMode").addEventListener("change",function(){ S.a11y.readingMode=this.checked; save(); applyA11y(); });
  if($("structureBtn")) $("structureBtn").addEventListener("click",function(e){ e.stopPropagation(); showPageStructure(); toggleA11yMenu(false); });
  if($("colorblindBtn")) $("colorblindBtn").addEventListener("click",function(){ cycleColorblind(); });
  if($("resetA11yBtn")) $("resetA11yBtn").addEventListener("click",function(){ resetA11yDefaults(); });
  if($("resetA11yMenuBtn")) $("resetA11yMenuBtn").addEventListener("click",function(){ resetA11yDefaults(); toggleA11yMenu(false); });
  if($("openA11yMenuBtn")) $("openA11yMenuBtn").addEventListener("click",function(){ toggleA11yMenu(true); $("a11yBtn").focus(); });
  $("voiceBtn").addEventListener("click",function(){ S.a11y.voice=!S.a11y.voice; save(); applyA11y(); toast(S.a11y.voice?(L()==="pt"?"🔊 Narração ligada":"🔊 Narration on"):(L()==="pt"?"🔈 Narração desligada":"🔈 Narration off")); });

  if($("onboardSkipBtn")) $("onboardSkipBtn").addEventListener("click",closeOnboarding);
  if($("onboardNextBtn")) $("onboardNextBtn").addEventListener("click",onboardNext);
  if($("onboardOpenBtn")) $("onboardOpenBtn").addEventListener("click",function(){ showOnboarding(true); });
  document.querySelectorAll(".onboard-lang-btn").forEach(function(b){ b.addEventListener("click",function(){ setLang(b.getAttribute("data-lang")); }); });
  if($("onboardOverlay")) $("onboardOverlay").addEventListener("click",function(e){ if(e.target===$("onboardOverlay")) closeOnboarding(); });
  document.addEventListener("keydown",function(e){ var ov=$("onboardOverlay"); if(e.key==="Escape"&&ov&&!ov.hidden) closeOnboarding(); });

  $("homeStartBtn").addEventListener("click",function(){ renderTeams(); renderRoles(); $("playerName").value=S.name||""; show("screenSetup"); });
  $("homeMapBtn").addEventListener("click",function(){ openMap(null,true); });
  if($("heroIronBtn")) $("heroIronBtn").addEventListener("click",function(){ openMap("iron",true); });
  $("setupBackBtn").addEventListener("click",function(){ show("screenHome"); });
  $("setupGoBtn").addEventListener("click",function(){ S.name=$("playerName").value.trim(); save(); openMap(null,true); });
  $("playerName").addEventListener("input",function(){ S.name=this.value; });

  $("mapBackBtn").addEventListener("click",function(){ show("screenHome"); });
  $("mapDetailClose").addEventListener("click",closeMapDetail);
  $("zoomIn").addEventListener("click",function(){ zoomTo(view.x+view.w/2,view.y+view.h/2,.8); });
  $("zoomOut").addEventListener("click",function(){ zoomTo(view.x+view.w/2,view.y+view.h/2,1.25); });
  $("zoomReset").addEventListener("click",function(){ resetView(); });

  /* Topbar: idioma + acessibilidade rápidos */
  $("langToggle").addEventListener("click",function(){ setLang(L()==="pt"?"en":"pt"); });
  $("a11yBtn").addEventListener("click",function(e){ e.stopPropagation(); toggleA11yMenu(); });
  $("a11yMenuClose").addEventListener("click",function(e){ e.stopPropagation(); toggleA11yMenu(false); });
  ...
  $("speakBtn").addEventListener("click",function(){ var q=cur.questions[cur.i]; if(q) speak(tt(q.q)); });
  $("nextBtn").addEventListener("click",nextQuestion);
  $("quitBtn").addEventListener("click",function(){ returnToMap(); });
  $("resultHomeBtn").addEventListener("click",function(){ show("screenHome"); });
  $("resultMapBtn").addEventListener("click",returnToMap);

  $("bossListBackBtn").addEventListener("click",function(){ show("screenHome"); });
  $("bossQuitBtn").addEventListener("click",function(){ renderBossList(); show("screenBossList"); });
  $("bossNext").addEventListener("click",bossNext);
  $("bossResultBackBtn").addEventListener("click",function(){ renderBossList(); show("screenBossList"); });
  if($("bossReplayBtn")) $("bossReplayBtn").addEventListener("click",function(){ if(bossCur.lastBossId) startBoss(bossCur.lastBossId); });

  $("dailyBackBtn").addEventListener("click",function(){ show("screenHome"); });
  $("dailyStartBtn").addEventListener("click",startDaily);
  $("weeklyBackBtn").addEventListener("click",function(){ show("screenHome"); });
  $("weeklyMapBtn").addEventListener("click",function(){ openMap(null,true); });

  $("shopBackBtn").addEventListener("click",function(){ show("screenHome"); });
  $("profileBackBtn").addEventListener("click",function(){ show("screenHome"); });
  if($("profileResetBtn")) $("profileResetBtn").addEventListener("click",resetProgress);
  if($("certGenerateBtn")) $("certGenerateBtn").addEventListener("click",renderCertificatePreview);
  if($("certDownloadBtn")) $("certDownloadBtn").addEventListener("click",downloadCertificate);
  if($("certPrintBtn")) $("certPrintBtn").addEventListener("click",printCertificate);
  $("mgrBackBtn").addEventListener("click",function(){ show("screenHome"); });
  $("mgrExportTeams").addEventListener("click",exportTeams);
  $("mgrExportThemes").addEventListener("click",exportThemes);

  if($("navHomeBtn")) $("navHomeBtn").addEventListener("click",function(){ show("screenHome"); });
  if($("navMapBtn")) $("navMapBtn").addEventListener("click",returnToMap);
  if($("navBossBtn")) $("navBossBtn").addEventListener("click",function(){ renderBossList(); show("screenBossList"); });
  if($("navDailyBtn")) $("navDailyBtn").addEventListener("click",function(){ renderDaily(); show("screenDaily"); });
  if($("navWeeklyBtn")) $("navWeeklyBtn").addEventListener("click",function(){ renderWeekly(); show("screenWeekly"); });
  if($("navShopBtn")) $("navShopBtn").addEventListener("click",function(){ renderShop(); show("screenShop"); });
  if($("navStatsBtn")) $("navStatsBtn").addEventListener("click",function(){ renderProfile(); show("screenProfile"); });
  if($("navManagerBtn")) $("navManagerBtn").addEventListener("click",function(){ renderManager(); show("screenManager"); });
  ...
  if($("mapSearch")) $("mapSearch").addEventListener("input",function(){ mapSearchQuery=this.value; applyMapSearch(); });
}
function init(){
  try{
  sanitizeA11y();
  applyI18n(); applyA11y(); applyCosmetics(); ensureDaily(); ensureWeekly(); ensureTeamScores(); ensureStreak(); refreshHud();
  renderStreakCard();
  updateHeroCaption();
  ...
  bind(); bindMapPanZoom(); ensureBossStats(); checkMedals(); showOnboarding();
